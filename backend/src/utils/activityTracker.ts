...
export async function awardXP(
    ...
    if (user.activityLog.length > 50) {
        user.activityLog = user.activityLog.slice(-50) as any;
    }
...
export async function trackStudySession(
    ...
    if (user.studyTime.sessions.length > 100) {
        user.studyTime.sessions = user.studyTime.sessions.slice(-100) as any;
    }
...
export async function getUserStats(userId: string) {
    ...
    return {
        ...
        longestStreak: user.streakData.longest,
        skillsMastered,
